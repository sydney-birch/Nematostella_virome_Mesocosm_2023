...
def count_reads(input_dir):
    ...
    for item in os.scandir():
        if "blastout" in item.name:
            sample_name = item.name
            #print("On Sample: ", sample_name)
            result = subprocess.run("echo -n \"{0} number of hits: \"".format(sample_name), shell=True)
            result = subprocess.run("cat {0} | wc -l".format(sample_name), shell=True)        
